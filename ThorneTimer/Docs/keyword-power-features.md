...
```
cost ~= sum chunks x (categories + activeTimers) x keywordsPerEntry x O(chunkLength)
```
...
|------|----------------|--------|---------------|-----------|
| **0 - Literal** | plain text (today) | `IndexOf(Ordinal[IgnoreCase])` | 1x (baseline) | No |
| **1 - Wildcard** | contains `*` and/or `?` (glob) | compiled `Regex`, cached | ~2-5x | Optional |
| **2 - Regex** | wrapped `^...$` or `/.../` escape hatch | compiled `Regex`, cached | ~3-8x | Yes |

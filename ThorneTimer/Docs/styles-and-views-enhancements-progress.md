...
| | |
|---|---|
| **[DONE] Done & committed** | Time Formats (section 2): `TimerTimeFormatter` + `TimeFormat` enum, `styles.TimeFormat` column + migration, Styles-tab dropdown with live `Example` preview, format-independent warning detection, grid Remaining column honoring per-style format. Spike deleted (harvested into Appendix A). Grid column-layout persistence pre-work (section 7.5) shipped in beta2. |
| **[IN FLIGHT] In flight (uncommitted)** | Interim time-parse hardening (section 13.7): `TimerPlus.TryParseRemaining`, `NormalizeRemainingForStorage`, dependency-delay fix. Stale-layout guard for newly-added grid columns. Opportunistic `FormMain` -> `TimersController` extraction (new file). |
| **[NOT STARTED] Not started** | All per-view `miniviews` fields (section 3.2). `ViewsRepository` field extensions (section 5). `StylePreviewPanel` + `StyleEditorDialog` + `ViewEditorDialog` + grid rework (sections 7.1-7.3). Settings Font Family picker (section 3.4 / 7.4). `IThorneMiniView` seam + `MiniViewFactory` (section 10). The **entire `ThorneView` skin engine** (section 8). |
| **[DEFERRED] Deferred (by design)** | Full numeric time-model refactor (section 13) - explicitly out of scope for this feature branch. |

---

## 2. Completion snapshot by plan section

| Plan section | Area | Status | Notes |
|--------|------|--------|-------|
...
| 5 | Repository changes | **[PARTIAL]** | `StylesRepository` extended for `TimeFormat`. `ViewsRepository` field extensions not done. |
...
| 12 | Acceptance / build verification | **[PARTIAL]** | Time-format acceptance criteria pass; renderer/opacity/header criteria N/A until section 8 lands. |
| 13 | Numeric time-model refactor | **[DEFERRED]** | Section 13.7 interim hardening in flight (see below); full refactor intentionally out of scope. |

Legend: [DONE] done - [PARTIAL] partial - [IN FLIGHT] in flight (uncommitted) - [NOT STARTED] not started - [DEFERRED] deferred
...
- [x] **Section 7.5 grid column-layout persistence** - Fill-aware load (skip pixel `Width` for Fill
...
These changes are **not yet committed**. They represent the section 13.7 interim hardening plus an
...
- [x] **Doc** - added section 13.7 "Interim hardening already landed" to the plan.

> **Action needed:** this is a meaningful, coherent chunk of work sitting uncommitted.
> Recommend committing it (e.g. `fix(timers): tolerant time parsing + dependency-delay fix; refactor(timers): extract TimersController`) before starting the next slice, so the working tree is clean for the section 3.2/8 work.
...
### Data / schema (sections 3.2, 3.4, 4, 5)
...
### Renderer + seam (sections 8, 10) - the largest remaining piece
...
- [ ] `SetAppearance(MiniViewAppearance)` on both renderers (section 8.1).

### Deferred (section 13) - out of scope for this branch
- [ ] Full numeric time-model refactor (`RemainingMs`/`TimeSpan` as source of truth, numeric
	  persistence, retire runtime string parsing). The section 13.7 interim fix removes the
...
|------|-----------|-------|
| `TimerTimeFormatter.cs` | section 2 formatter | [DONE] exists (committed) |
| `Styles.cs` (`StyleData`) | section 3.1 | [PARTIAL] `TimeFormat` only |
| `StylesRepository.cs` | section 5 | [PARTIAL] `TimeFormat` only |
| `StylesController.cs` | section 7.3 | [PARTIAL] inline Time Format column + Example preview |
| `TimerPlus.cs` | section 6 / 13.7 | [IN FLIGHT] formatter delegation committed; `TryParseRemaining` uncommitted |
| `TimerRuntime.cs` | section 6 / 13.7 | [IN FLIGHT] resolver committed; dependency fix uncommitted |
| `TimerStateRepository.cs` | section 13.7 | [IN FLIGHT] normalize-on-store uncommitted |
| `GridLayoutManager.cs` | section 7.5 | [IN FLIGHT] stale-layout guard uncommitted |
| `TimersController.cs` | (opportunistic) | [IN FLIGHT] new, untracked |
| `Views.cs` (`ViewData`) | section 3.2 | [NOT STARTED] unchanged |
| `ViewsRepository.cs` | section 5 | [NOT STARTED] no new fields |
| `IThorneMiniView.cs` | section 10 | [NOT STARTED] does not exist |
| `MiniViewFactory.cs` | section 10 | [NOT STARTED] does not exist |
| `ThorneView.cs` / `.Designer.cs` | section 8 | [NOT STARTED] do not exist |
| `StylePreviewPanel.cs` | section 7.1 | [NOT STARTED] does not exist |
| `StyleEditorDialog.cs` | section 7.2 | [NOT STARTED] does not exist |
| `ViewEditorDialog.cs` | section 7.2 | [NOT STARTED] does not exist |
| `Spike\LayeredMiniViewSpike.cs` | harvested | [DONE] deleted (as planned) |

---

## 7. Recommended resumption order

1. **Commit the in-flight work** (section 13.7 hardening + `GridLayoutManager` guard + `TimersController`
   extraction) so the tree is clean. Build + `get_errors` clean first.
2. **Schema + repos for views (sections 3.2, 4, 5)** - add the `miniviews` columns (idempotent,
...
3. **Dual-renderer seam (section 10.3 steps 1-2)** - extract `IThorneMiniView`, make `MiniView`
...
4. **`ThorneView` skin engine (section 8)** - build against the interface using Appendix A plumbing,
   Classic untouched and default.
5. **UI rework (sections 7.1-7.4)** - `StylePreviewPanel`, then the two editor dialogs, then grid
   rework, then the Settings Font Family picker.
6. **Cutover (section 10.3 step 5)** - only after dogfooding parity, flip the default; later remove
   `MiniView`.
...
From plan section 9 (most are resolved; these remain):

- **Section 9.6 - Default engine cutover.** Confirmed approach: keep **Classic** as the default
...
- **Color palette final cross-check (section 11).** Defaults were aligned in the per-view-colors work;
...
- All four `TimeFormat`s render per the section 2.1 table for sub-minute / multi-hour / multi-day spans.
- Warning threshold fires correctly under **every** format (the section 2.5 regression) - verify
...
- **Dependency chains** with a non-Classic style (e.g. Spawn = `FullCompact`) honor
  `DependsOnDelay` (the section 13.7 bug - chain must stagger, not fire all at once).
